...
class EntrySF:
    ...
    def is_deleted(self) -> bool:
        ...
        return self.offset == DELETED_PTR
...
class SequentialIndex:
    ...
    def __init__(self, table: str, column: str, data_dir: Optional[str] = None, column_type: Optional[str] = None, idx_name: str | None = None):
        # support calls like SequentialIndex(table, column, data_dir=path)
        base_dir = os.path.abspath(data_dir) if data_dir else os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "data", "idx_sequential"))
        os.makedirs(base_dir, exist_ok=True)

        # file name: allow idx_name override, otherwise use table_column
        if idx_name:
            self.filename = os.path.join(base_dir, f"{table}_{idx_name}.dat")
        else:
            self.filename = os.path.join(base_dir, f"{table}__{column}.sidx")

        # determine storage kind: accept either '64s' or 's64' (legacy)
        if column_type is None:
            column_type = "64s"

        # normalize string type representations so struct expects '<i{N}s i'
        if column_type == "i":
            self.value_kind = "i"
            self.value_size = 4
        elif column_type == "f":
            self.value_kind = "f"
            self.value_size = 4
        elif isinstance(column_type, str) and column_type.endswith("s"):
            # allow '64s' or legacy 's64'
            body = column_type[:-1]
            if body.isdigit():
                size = int(body)
            elif column_type.startswith("s") and column_type[1:-1].isdigit():
                # handles 's64'
                size = int(column_type[1:-1] if column_type.endswith("s") else column_type[1:])
            else:
                size = 64
            self.value_kind = f"{size}s"
            self.value_size = size
        else:
            # default to fixed-size 64-byte string
            self.value_kind = "64s"
            self.value_size = 64

        self.emp_format = f"<i{self.value_kind}i"
    ...
    def _cmp(self, a, b) -> int:
        """Compare a and b robustly. Return negative if a<b, 0 if equal, positive if a>b.
        Tries numeric comparison when both look numeric, otherwise falls back to string compare.
        """
        # normalize None
    ...
    def search_range(self, lo, hi) -> List[int]:
        ...
        with open(self.filename, "rb") as f:
            ...
            while not is_end(cur):
                ...
                if self._cmp(node.value, hi) > 0:
                    break

                if self._cmp(node.value, lo) >= 0:
    ...
    def search(self, key) -> List[int]:
        ...
        """
        # guard: if key is None, nothing to search
        if key is None:
            return []
        ...
        with open(self.filename, "rb") as f:
            ...
            while cur != 0:
                ...
                if node.is_deleted():
                    cur = node.next_ptr
                    continue
                if self._cmp(node.value, key) == 0:
                    ans.append(node.offset)
                elif self._cmp(node.value, key) > 0:
                    break
                cur = node.next_ptr
        return ans
        ...
        with open(self.filename, "r+b") as f:
            ...
            while not is_end(cur_ptr):
                ...
                if node.is_deleted():
                    ...
                    continue
                if self._cmp(node.value, emp.value) < 0:
                    prev_ptr = cur_ptr
                    cur_ptr = node.next_ptr
                else:
                    break
    ...
    def delete(self, key) -> List[int]:
        ...
        with open(self.filename, "r+b") as f:
            ...
            while not is_end(cur_ptr):
                ...
                if self._cmp(node.value, key) > 0:
                    break
                if self._cmp(node.value, key) == 0 and not node.is_deleted():
                    orig = node.offset
                    node.offset = DELETED_PTR
    ...
    def reorganize(self):
        ...
        # Reemplazo atómico con reintento en Windows si hay PermissionError
        try:
            os.replace(tmp_path, self.filename)
        except PermissionError:
            # intentar forzar permisos y reemplazar: útil en Windows si el archivo destino está marcado read-only
            try:
                ...
                # último recurso: intenta renombrar (no garantiza atomicidad)
                try:
                    os.rename(tmp_path, self.filename)
                except Exception:
                    # si todo falla, propaga el error para que el caller lo vea
                    raise e

    # backward compatibility: some callers expect a `reconstruct()` method
    def reconstruct(self):
        """Alias for reorganize() to match older API names."""
        return self.reorganize()

    # Compatibility wrappers ------------------------------------------------
    def add(self, key, offset) -> None:
        return self.insert(key, offset)

    def find(self, key) -> List[int]:
        return self.search(key)

    def range_search(self, begin_key, end_key) -> List[int]:
        return self.search_range(begin_key, end_key)

    def remove(self, key) -> List[int]:
        return self.delete(key)

    def count(self) -> int:
        m, a, _ = self._get_header()
        return m + a

    def info(self) -> dict:
        m, a, h = self._get_header()
        return {"table": os.path.basename(self.filename), "main_count": m, "aux_count": a, "head_ptr": h}